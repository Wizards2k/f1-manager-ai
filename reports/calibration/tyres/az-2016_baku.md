# Tyre calibration – az-2016_baku

## Track stats
- avg_heat: 0.748
- avg_bump: 0.000
- total_brake_mj: 16.684
- brake_density_mj_per_km: 2.810

## Compounds
### C1
| Parametro | Valore |
|-----------|--------|
| temp_window_surface_c | [88.74, 123.74, 138.74] |
| temp_window_core_c | [89.24, 101.24, 114.24] |
| gaussian_sigma_surface_c | 7.0 |
| gaussian_sigma_core_c | 6.0 |
| base_grip | 0.92 |
| wear_rate_base_pct_per_km | 0.088192 |
| degradation_rate_multiplier | 0.6 |
| slip_sensitivity | 0.75 |
| thermal_mass_surface | 1.25 |
| thermal_mass_core | 1.4 |
| conduction_coeff | 0.06 |
| cooling_coeff | 1.05 |

### C2
| Parametro | Valore |
|-----------|--------|
| temp_window_surface_c | [88.74, 120.74, 133.74] |
| temp_window_core_c | [87.24, 98.24, 109.24] |
| gaussian_sigma_surface_c | 7.0 |
| gaussian_sigma_core_c | 6.0 |
| base_grip | 0.95 |
| wear_rate_base_pct_per_km | 0.10779 |
| degradation_rate_multiplier | 0.8 |
| slip_sensitivity | 0.8 |
| thermal_mass_surface | 1.18 |
| thermal_mass_core | 1.32 |
| conduction_coeff | 0.065 |
| cooling_coeff | 1.05 |

### C3
| Parametro | Valore |
|-----------|--------|
| temp_window_surface_c | [86.74, 118.74, 133.74] |
| temp_window_core_c | [84.24, 96.24, 107.24] |
| gaussian_sigma_surface_c | 7.0 |
| gaussian_sigma_core_c | 6.0 |
| base_grip | 1.0 |
| wear_rate_base_pct_per_km | 0.127388 |
| degradation_rate_multiplier | 1.0 |
| slip_sensitivity | 1.0 |
| thermal_mass_surface | 1.1 |
| thermal_mass_core | 1.25 |
| conduction_coeff | 0.07 |
| cooling_coeff | 1.0 |

### C4
| Parametro | Valore |
|-----------|--------|
| temp_window_surface_c | [83.74, 106.74, 118.74] |
| temp_window_core_c | [79.24, 91.24, 99.24] |
| gaussian_sigma_surface_c | 7.0 |
| gaussian_sigma_core_c | 6.0 |
| base_grip | 1.06 |
| wear_rate_base_pct_per_km | 0.156785 |
| degradation_rate_multiplier | 1.3 |
| slip_sensitivity | 1.15 |
| thermal_mass_surface | 0.98 |
| thermal_mass_core | 1.12 |
| conduction_coeff | 0.075 |
| cooling_coeff | 0.95 |

### C5
| Parametro | Valore |
|-----------|--------|
| temp_window_surface_c | [80.74, 100.74, 113.74] |
| temp_window_core_c | [74.24, 87.24, 94.24] |
| gaussian_sigma_surface_c | 7.0 |
| gaussian_sigma_core_c | 6.0 |
| base_grip | 1.12 |
| wear_rate_base_pct_per_km | 0.186182 |
| degradation_rate_multiplier | 1.6 |
| slip_sensitivity | 1.3 |
| thermal_mass_surface | 0.9 |
| thermal_mass_core | 1.05 |
| conduction_coeff | 0.08 |
| cooling_coeff | 0.92 |

### C6
| Parametro | Valore |
|-----------|--------|
| temp_window_surface_c | [78.74, 94.74, 103.74] |
| temp_window_core_c | [69.24, 83.24, 89.24] |
| gaussian_sigma_surface_c | 7.0 |
| gaussian_sigma_core_c | 6.0 |
| base_grip | 1.18 |
| wear_rate_base_pct_per_km | 0.21558 |
| degradation_rate_multiplier | 1.8 |
| slip_sensitivity | 1.45 |
| thermal_mass_surface | 0.82 |
| thermal_mass_core | 0.98 |
| conduction_coeff | 0.085 |
| cooling_coeff | 0.9 |

### INTERMEDIATE
| Parametro | Valore |
|-----------|--------|
| temp_window_surface_c | [58.74, 70.74, 83.74] |
| temp_window_core_c | [54.24, 67.24, 74.24] |
| gaussian_sigma_surface_c | 7.5 |
| gaussian_sigma_core_c | 6.5 |
| base_grip | 0.95 |
| wear_rate_base_pct_per_km | 0.117589 |
| degradation_rate_multiplier | 0.9 |
| slip_sensitivity | 0.9 |
| thermal_mass_surface | 1.05 |
| thermal_mass_core | 1.2 |
| conduction_coeff | 0.065 |
| cooling_coeff | 1.1 |

### WET
| Parametro | Valore |
|-----------|--------|
| temp_window_surface_c | [38.74, 53.74, 68.74] |
| temp_window_core_c | [34.24, 49.24, 59.24] |
| gaussian_sigma_surface_c | 8.0 |
| gaussian_sigma_core_c | 7.0 |
| base_grip | 0.85 |
| wear_rate_base_pct_per_km | 0.097991 |
| degradation_rate_multiplier | 0.7 |
| slip_sensitivity | 0.85 |
| thermal_mass_surface | 1.15 |
| thermal_mass_core | 1.3 |
| conduction_coeff | 0.06 |
| cooling_coeff | 1.2 |
