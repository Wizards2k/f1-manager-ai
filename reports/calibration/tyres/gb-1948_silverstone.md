# Tyre calibration – gb-1948_silverstone

## Track stats
- avg_heat: 0.731
- avg_bump: 0.000
- total_brake_mj: 9.900
- brake_density_mj_per_km: 1.696

## Compounds
### C1
| Parametro | Valore |
|-----------|--------|
| temp_window_surface_c | [88.65, 123.65, 138.65] |
| temp_window_core_c | [89.19, 101.19, 114.19] |
| gaussian_sigma_surface_c | 7.0 |
| gaussian_sigma_core_c | 6.0 |
| base_grip | 0.92 |
| wear_rate_base_pct_per_km | 0.088065 |
| degradation_rate_multiplier | 0.6 |
| slip_sensitivity | 0.75 |
| thermal_mass_surface | 1.25 |
| thermal_mass_core | 1.4 |
| conduction_coeff | 0.06 |
| cooling_coeff | 1.05 |

### C2
| Parametro | Valore |
|-----------|--------|
| temp_window_surface_c | [88.65, 120.65, 133.65] |
| temp_window_core_c | [87.19, 98.19, 109.19] |
| gaussian_sigma_surface_c | 7.0 |
| gaussian_sigma_core_c | 6.0 |
| base_grip | 0.95 |
| wear_rate_base_pct_per_km | 0.107635 |
| degradation_rate_multiplier | 0.8 |
| slip_sensitivity | 0.8 |
| thermal_mass_surface | 1.18 |
| thermal_mass_core | 1.32 |
| conduction_coeff | 0.065 |
| cooling_coeff | 1.05 |

### C3
| Parametro | Valore |
|-----------|--------|
| temp_window_surface_c | [86.65, 118.65, 133.65] |
| temp_window_core_c | [84.19, 96.19, 107.19] |
| gaussian_sigma_surface_c | 7.0 |
| gaussian_sigma_core_c | 6.0 |
| base_grip | 1.0 |
| wear_rate_base_pct_per_km | 0.127205 |
| degradation_rate_multiplier | 1.0 |
| slip_sensitivity | 1.0 |
| thermal_mass_surface | 1.1 |
| thermal_mass_core | 1.25 |
| conduction_coeff | 0.07 |
| cooling_coeff | 1.0 |

### C4
| Parametro | Valore |
|-----------|--------|
| temp_window_surface_c | [83.65, 106.65, 118.65] |
| temp_window_core_c | [79.19, 91.19, 99.19] |
| gaussian_sigma_surface_c | 7.0 |
| gaussian_sigma_core_c | 6.0 |
| base_grip | 1.06 |
| wear_rate_base_pct_per_km | 0.15656 |
| degradation_rate_multiplier | 1.3 |
| slip_sensitivity | 1.15 |
| thermal_mass_surface | 0.98 |
| thermal_mass_core | 1.12 |
| conduction_coeff | 0.075 |
| cooling_coeff | 0.95 |

### C5
| Parametro | Valore |
|-----------|--------|
| temp_window_surface_c | [80.65, 100.65, 113.65] |
| temp_window_core_c | [74.19, 87.19, 94.19] |
| gaussian_sigma_surface_c | 7.0 |
| gaussian_sigma_core_c | 6.0 |
| base_grip | 1.12 |
| wear_rate_base_pct_per_km | 0.185915 |
| degradation_rate_multiplier | 1.6 |
| slip_sensitivity | 1.3 |
| thermal_mass_surface | 0.9 |
| thermal_mass_core | 1.05 |
| conduction_coeff | 0.08 |
| cooling_coeff | 0.92 |

### C6
| Parametro | Valore |
|-----------|--------|
| temp_window_surface_c | [78.65, 94.65, 103.65] |
| temp_window_core_c | [69.19, 83.19, 89.19] |
| gaussian_sigma_surface_c | 7.0 |
| gaussian_sigma_core_c | 6.0 |
| base_grip | 1.18 |
| wear_rate_base_pct_per_km | 0.21527 |
| degradation_rate_multiplier | 1.8 |
| slip_sensitivity | 1.45 |
| thermal_mass_surface | 0.82 |
| thermal_mass_core | 0.98 |
| conduction_coeff | 0.085 |
| cooling_coeff | 0.9 |

### INTERMEDIATE
| Parametro | Valore |
|-----------|--------|
| temp_window_surface_c | [58.65, 70.65, 83.65] |
| temp_window_core_c | [54.19, 67.19, 74.19] |
| gaussian_sigma_surface_c | 7.5 |
| gaussian_sigma_core_c | 6.5 |
| base_grip | 0.95 |
| wear_rate_base_pct_per_km | 0.11742 |
| degradation_rate_multiplier | 0.9 |
| slip_sensitivity | 0.9 |
| thermal_mass_surface | 1.05 |
| thermal_mass_core | 1.2 |
| conduction_coeff | 0.065 |
| cooling_coeff | 1.1 |

### WET
| Parametro | Valore |
|-----------|--------|
| temp_window_surface_c | [38.65, 53.65, 68.65] |
| temp_window_core_c | [34.19, 49.19, 59.19] |
| gaussian_sigma_surface_c | 8.0 |
| gaussian_sigma_core_c | 7.0 |
| base_grip | 0.85 |
| wear_rate_base_pct_per_km | 0.09785 |
| degradation_rate_multiplier | 0.7 |
| slip_sensitivity | 0.85 |
| thermal_mass_surface | 1.15 |
| thermal_mass_core | 1.3 |
| conduction_coeff | 0.06 |
| cooling_coeff | 1.2 |
